...
export default {
    ...
    data() {
        return {
            ...
            rooms: ["Room 0", "Room 1", "Room 2", "Room 3", "Room 4", "Room 5", "Room 6", "Room 7", "Room 8", "Room 9"],
            accessTokens: [
    ...
    methods: {
        ...
        async start() {
            ...
            result = await this.connection.invoke("Join", {
                ...
                sctpCapabilities:
                    this.form.useDataChannel && this.form.consume ? this.mediasoupDevice.sctpCapabilities : undefined,
                displayName: null,
        ...
        async handleJoinRoom() {
            ...
            if (this.form.produce) {
                ...
                result = await this.connection.invoke("CreateSendWebRtcTransport", {
                    ...
                    sctpCapabilities: this.form.useDataChannel ? this.mediasoupDevice.sctpCapabilities : undefined,
                });
                if (result.code !== 200) {
                ...
                dataConsumer.on("message", (message) => {
                    ...
                    if (message instanceof ArrayBuffer) {
                        ...
                        if (number > this.nextDataChannelTestNumber) {
                            logger.warn("dataChannelTest: %s packets missing", number - this.nextDataChannelTestNumber);
                        }
        ...
        async enableCam() {
            ...
            try {
                ...
                if (this.useSimulcast) {
                    ...
                    if (firstVideoCodec.mimeType.toLowerCase() === "video/vp9") encodings = WEBCAM_KSVC_ENCODINGS;
